...
if(!mute){
        ...
        io.sockets.emit('add mess', {user:userName, msg: data, time: Date.now()});
                          }else{
   ...
   //   }
    ...
    socket.on('getHistory', function(data){
            ...
            Message.find({}).sort({$natural:-1}).limit(perPage)
            ...
            if(!history){
                ...
                io.sockets.emit('loadHistory', {history:history, login:userName});
            }
            }) 
